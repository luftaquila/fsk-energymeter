...
function splitRange(dMin, dMax) {
  if (dMin === dMax) { dMin *= 0.85; dMax *= 1.15 }
  else { const r = dMax - dMin; dMin -= r * 0.05; dMax += r * 0.05 }
...
function initChart() {
  ...
  for (const k of Object.keys(axis)) {
    scales[k] = { range: (u, dMin, dMax) => { if (dMin === null && dMax === null) return [null, null]; axis[k] = splitRange(dMin, dMax); return [axis[k].min, axis[k].max] } }
  }
  const fmt = (v) => { if (!v) return '-'; const d = new Date(v), p = n => String(n).padStart(2, '0'); return `${p(d.getHours())}:${p(d.getMinutes())}:${p(d.getSeconds())}.${String(d.getMilliseconds()).padStart(3, '0')}` }

  uplot = new uPlot({
    width: chartContainer.value.clientWidth - 32, height: 500, ms: true,
    series: [
      { value: (_, v) => fmt(v) },
      ...
      { label: '500ms', scale: 'kW', points: { show: true, size: 6, fill: 'darkgray', stroke: 'darkgray', width: 2 } }
    ],
    axes: [
      { stroke: '#999', values: (_, t) => t.map(v => { const d = new Date(v), p = n => String(n).padStart(2, '0'); return `${p(d.getHours())}:${p(d.getMinutes())}:${p(d.getSeconds())}\n${String(d.getMilliseconds()).padStart(3, '0')}` }) },
      { scale: 'A', stroke: 'dodgerblue', values: (_, t) => t.map(v => v.toFixed(1) + 'A'), splits: () => axis.A.splits, size: 55 },
      { scale: 'HV', stroke: 'red', values: (_, t) => t.map(v => v.toFixed(1) + 'V'), splits: () => axis.HV.splits, size: 55 },
      { scale: 'kW', stroke: 'mediumorchid', values: (_, t) => t.map(v => v.toFixed(1) + 'kW'), side: 1, splits: () => axis.kW.splits, size: 60 },
      { scale: 'LV', stroke: 'green', values: (_, t) => t.map(v => v.toFixed(1) + 'V'), side: 1, splits: () => axis.LV.splits, size: 55 },
      { scale: 'C', stroke: 'orange', values: (_, t) => t.map(v => v.toFixed(1) + '°C'), side: 1, splits: () => axis.C.splits, size: 55 }
    ],
    scales,
    plugins: [touchZoomPlugin(), wheelZoomPlugin({ factor: 0.75 }), peakAnnotationsPlugin(result), violationVisibilityPlugin()]
  }, null, chartContainer.value)
}
...
function handleFileSelect(e) {
  ...
  if (ext === 'log') {
    ...
    reader.onload = (e) => { try { result.value = parse(new Uint8Array(e.target.result)); setChartData(calculateMetadata(result.value, powerLimit.value)) } catch (err) { uplot?.setData([]); alerts.value.errors = [err.message]; notyf.error('Failed to parse file') } }
  } else if (ext === 'csv' || ext === 'json') {
    reader.readAsText(file)
    reader.onload = (e) => {
      try {
        if (ext === 'csv') { const lines = e.target.result.split('\n'); const idx = lines.indexOf('original json data'); if (idx === -1 || !lines[idx + 1]) throw new Error('Cannot restore JSON from CSV'); result.value = JSON.parse(lines[idx + 1]) }
        else result.value = JSON.parse(e.target.result)
        setChartData(calculateMetadata(result.value, powerLimit.value))
      } catch (err) { uplot?.setData([]); alerts.value.errors = [err.message]; notyf.error('Failed to parse file') }
    }
  } else notyf.error('Unsupported file format')
}

function setChartData(data) { uplot?.setData(data.processed); displayMetadata(data) }
...
function displayMetadata(logs) {
  ...
  alerts.value.warnings = logs.header.datetime > Number(new Date(2099, 0)) ? ['Invalid RTC date detected. Sync the clock in the Device configuration tab.'] : []
...
onUnmounted(() => { window.removeEventListener('resize', handleResize); uplot?.destroy() })
</script>
  ...
  <div class="data-viewer">
    ...
    <div class="card fade-in" style="animation-delay:0.1s">
      ...
      <div class="card-body">
        ...
        <div v-if="alerts.warnings.length" class="alert alert-warning"><div v-for="(w,i) in alerts.warnings" :key="i">{{ w }}</div></div>
        <div v-if="alerts.errors.length" class="alert alert-danger"><div v-for="(e,i) in alerts.errors" :key="i">{{ e }}</div></div>
...
<style scoped>
.data-viewer { display: flex; flex-direction: column; gap: 1.5rem; }
.file-info { margin-bottom: 1rem; font-size: 0.875rem; }
.file-info .label { color: var(--text-secondary); margin-right: 0.5rem; }
...
.chart-hint i { margin-right: 0.5rem; }
</style>
